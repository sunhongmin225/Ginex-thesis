...
torch::Tensor load_int32(std::string file, int64_t size){

    // open file
    int fd = open(file.c_str(), O_RDONLY | O_DIRECT);

    int64_t result_buffer_size = size*sizeof(int32_t)+ALIGNMENT;
    int64_t num_blocks = result_buffer_size / ALIGNMENT;
    int32_t* result_buffer = (int32_t*)aligned_alloc(ALIGNMENT, result_buffer_size);

    #pragma omp parallel for num_threads(atoi(getenv("GINEX_NUM_THREADS")))
    for (int64_t n = 0; n < num_blocks; n++) {
        int64_t offset = n*ALIGNMENT;
        
        if (pread(fd, result_buffer+(ALIGNMENT/sizeof(int32_t))*n, ALIGNMENT, offset) == -1){
            fprintf(stderr, "load.cpp::2::ERROR: %s\n", strerror(errno));
        }
    }
...
PYBIND11_MODULE(mt_load, m) {
	...
	m.def("load_int32", &load_int32, "multi-threaded load (int32)");
}
