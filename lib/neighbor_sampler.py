...
def sample_adj_int32_ginex(rowptr, col, subset, num_neighbors, replace, cache_data=None, address_table=None):
    rowptr, col, n_id, indptr, num_hit, num_miss = sample.sample_adj_int32_ginex(
        rowptr, col, subset, cache_data, address_table, num_neighbors, replace)                                 
    out = SparseTensor(rowptr=rowptr, row=None, col=col,                                                 
                       sparse_sizes=(subset.size(0), n_id.size(0)),                                      
                       is_sorted=True)                                                                   
