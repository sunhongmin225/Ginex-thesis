...
int main(void) {
  ...
  int num_elems = 7;
  int64_t* src_int64 = (int64_t*) malloc(num_elems * sizeof(int64_t));
  src_int64[0] = 912309;
  src_int64[1] = 334234;
  src_int64[2] = 192933;
  src_int64[3] = 231233;
  ...
  const int compressed_data_size = LZ4_compress_default((const char*) src_int64, (char*) compressed_data, src_size, max_dst_size);
  ...
  ret = fwrite(compressed_data, 1, compressed_data_size, fw);
  ...
  /* Read from file*/
  FILE *fr;
  int64_t* comp_buffer = (int64_t*) malloc(compressed_data_size);
  fr = fopen("toy_lz4.dat.lz4", "rb"); // "a"
  ret = fread(comp_buffer, 1, compressed_data_size, fr);
  printf("arcmsh::read file with ret = %ld\n", ret);
  fclose(fr);

  printf("arcmsh::successfully read file\n");
